...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    lazy val neighbors = b.legalNeighbors.toStream map (pair => (pair._1, pair._2 :: history))
    neighbors
  }
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    lazy val newNeighbors = neighbors filter (pair => !explored.contains(pair._1))
    newNeighbors
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, moves) #:: tail => {
      ...
      (block, moves) #:: from(tail ++ ys, explored + block)
    }
    case _ => Stream.empty
  }
